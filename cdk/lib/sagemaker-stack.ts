...
export class SageMakerStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id:string) {
    ...
    const recommendationsLambdaCodeAsset = assetLoader.getAssetPath("lambda-recommendations/service");
    this.patchLambda(recommendationsLambdaCodeAsset, {
      sagemakerEndpoint: 'REPLACE_ME_SAGEMAKER_ENDPOINT'  // FIXME: pull this from somewhere
    });
    const mysfitsRecommendations = new lambda.Function(this, "Function", {
      ...
      memorySize: 128,
      code: lambda.Code.fromAsset(recommendationsLambdaCodeAsset),
  ...
  private patchLambda(root: string, params: {sagemakerEndpoint: string}): void {
    const template = assetLoader.getTemplatePath('lambda-recommendations/recommendations.py');
    const outfile = `${root}/recommendations.py`;
    const content = fs.readFileSync(template).toString()
                        .replace(/REPLACE_ME_SAGEMAKER_ENDPOINT_NAME/, params.sagemakerEndpoint);

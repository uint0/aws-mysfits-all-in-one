...
stack_kinesis=$(get_mm_stack_arn KinesisFirehose)
stack_xray=$(get_mm_stack_arn XRay)
...
streamendpoint=$(get_mm_stack_output_fuzzy $stack_kinesis APIEndpoint)
questionendpoint=$(get_mm_stack_output_fuzzy $stack_xray APIEndpoint)
...
sed -i "s/REPLACE_ME_STREAM_EP/${streamendpoint//\//\\/}/g"     config.js
sed -i "s/REPLACE_ME_QUESTION_EP/${questionendpoint//\//\\/}/g" config.js
